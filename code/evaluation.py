...
def get_relation_embeddings(model, loader, device):
    all_rel_embs = []
    y_true       = []
    ...
    for data in tqdm(loader):
        ...
        with torch.no_grad():
            ...
            rel_embs = outputs_dict['relation_embeddings']
        
        all_rel_embs.extend(list(np.array(rel_embs.cpu().detach())))
        y_true.extend(list(np.array(labels.cpu().detach())))
        
        # import pdb; pdb.set_trace()
        
    return all_rel_embs, y_true
