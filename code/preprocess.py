        ...
        for count, doc in enumerate(docs):
            ...
            for rel_int in interdict:
                ...
                for sent in interdict[rel_int]:
                    if sent_start is None: sent_start = sent.start_char
                ...
                assert sent_start <= min(arg1_start, arg2_start) and sent_end >= max(arg2_end, arg1_end)
    
                e1_start, e1_end, e2_start, e2_end = arg1_start - sent_start, arg1_end - sent_start, arg2_start - sent_start, arg2_end - sent_start
                ...
                e1_toks											= [0]+[1 if arg1_ann_map.contains(elem[0],elem[1]) else 0 for elem in tok_range[1:-1]]+[0]
                e2_toks 										= [0]+[1 if arg2_ann_map.contains(elem[0],elem[1]) else 0 for elem in tok_range[1:-1]]+[0]

                assert 1 in e1_toks; assert 1 in e2_toks
                ...
                for sent_cnt, dep_sent in enumerate(dep_doc.sentences):
                    for word in dep_sent.words:
                        if     arg1_ann_map.contains(word.start_char, word.end_char) and  arg2_ann_map.contains(word.start_char, word.end_char): dep_val =3
                        elif   arg2_ann_map.contains(word.start_char, word.end_char): dep_val =2
                        elif   arg1_ann_map.contains(word.start_char, word.end_char): dep_val =1 
                        else:  dep_val =0
                ...
                for node in node_dict:
                    ...
                    if mask == 3: n1_mask.append(1); n2_mask.append(1)
...
def create_parses():
    if  args.dataset == 'risec': 	
        if args.parse == 'srl':
            ...
            dump_amrs('../data/risec/',['train','dev','test'], bert_model ='bert-base-uncased', text_tokenizer='scispacy')
...
def load_dataset():
    ...
    if args.dataset == 'japflow':
        dataset 	= create_datafield(f'../data/{args.dataset}/',['all'], bert_model ='bert-base-uncased', text_tokenizer='scispacy')
        dump_dill(dataset, f'../data/{args.dataset}/data.dill')
