...
def annotate_corpus(corpus_name, annotation_service_port):
    ...
    for item in tqdm(corpus):
        for sentence in sentence_tokenizer(item):
            sentence_annotation = annotate_text(
                sentence, ANNOTATION_SERVICE_URL.format(annotation_service_port)
            )
            annotations.append({"text": sentence, "annotations": sentence_annotation})
