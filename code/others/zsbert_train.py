...
def get_args():
	...
	parser.add_argument("--domain",        	help="choice of whether to obtain the labels",       		type=str, default='tgt')
...
def main(args):
	...
	if check_file(tgt_file):
		tgt_dataset,_							=   load_dill(tgt_file)
	else:
		tgt_dataset,_			            	=   create_datafield(tgt_dir,['test'],args.bert_model, omit_rels= omit_rels) #omit_rels=['t-part-of','f-set','f-comp','f-part-of','t-eq','v-tm','f-eq']
	...
	trainloader 												= DataLoader(trainset, batch_size=args.batch_size, collate_fn=create_mini_batch_orig, shuffle=True)
	model.train()
	...
	if args.mode 												== 'train':
		...
		devloader     							   				= DataLoader(devset, batch_size=args.batch_size, collate_fn=create_mini_batch_orig)
		kill_cnt 												= 0
		for epoch in range(args.epochs):
			...
			for data in tqdm(trainloader):
				tokens_tensors, segments_tensors, marked_e1, marked_e2, masks_tensors, relation_emb, labels = [t.to(device) for t in data]
				optimizer.zero_grad()
	...
	if args.mode												 == 'eval':
		...
		testset        							   					 = ZSBertRelDataset(test_data, test_lbl2id, tokenizer, args, domain=args.domain) # 'tgt
		testloader     							   					 = DataLoader(testset, batch_size=args.batch_size, collate_fn=create_mini_batch_orig)
