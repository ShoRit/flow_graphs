...
from transformers import BertModel, BertConfig, BertPreTrainedModel, BertTokenizer, AutoTokenizer
...
def main(args):
	...
	if check_file(src_file):
		...
		src_dataset			            		=   create_datafield(src_dir,['train','dev','test'],tokenizer)
	...
	if args.mode 												 == 'train':
		...
		devset        							   				= ZSBertRelDataset(dev_data, dev_lbl2id, tokenizer, args, domain='tgt')
	...
	if args.mode												 == 'eval':
		...
		testset        							   					 = ZSBertRelDataset(test_data, test_lbl2id, tokenizer, args, domain='tgt')
