...
class GNNConv(torch.nn.Module):
	def __init__(self, params, num_rels):
		...
		if self.params.gnn == 'rgcn':
			self.gnn 	= RGCNConv(in_channels= self.params.node_emb_dim, out_channels=self.params.node_emb_dim, num_relations=num_rels)
		elif self.params.gnn == 'rgat':
			self.gnn 	= RGATConv(in_channels= self.params.node_emb_dim, out_channels=self.params.node_emb_dim, num_relations=num_rels)
