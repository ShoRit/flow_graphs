...
from transformers import AutoTokenizer, BertModel, BertConfig, BertPreTrainedModel, BertTokenizer
...
def get_args():
    ...
    parser.add_argument("--fewshot",        help="choice of the fewshot",       type=float, default =0.1)
...
def main(args):
    ...
    src_train_data                              =   src_dataset['train']['rels']
    train_data, dev_data, test_data     	    =   tgt_dataset['train']['rels'], tgt_dataset['dev']['rels'], tgt_dataset['test']['rels']
    ...
    src_bertconfig                              = BertConfig.from_pretrained(args.bert_model,num_labels=len(set(src_train_label)))
    tgt_bertconfig                              = BertConfig.from_pretrained(args.bert_model,num_labels=len(set(tgt_train_label)))
    
    src_bertconfig                              = create_bertconfig(src_bertconfig, args)
    tgt_bertconfig                              = create_bertconfig(tgt_bertconfig, args)
    

    if args.domain 														  == 'src':
        labels 															  = sorted(list(set(tgt_train_label)))
    ...
    src_model_dict                                                     = {k: v for k, v in src_model_dict.items() if k not in ['rel_classifier.weight', 'rel_classifier.bias']}
    model_dict.update(src_model_dict)
    ...
    if args.mode 													    == 'train':
        wandb.login()
        wandb.init(project="narrative-flow", entity="flow-graphs-cmu", name=f'{tgt_checkpoint_file.split("/")[-1]}')
        ...
        wandb.log({"fewshot":  args.fewshot})
        ...
        for epoch in range(args.epochs):
            ...
            for data in tqdm(trainloader):
                ...
                optimizer.zero_grad()
                output_dict 	 = model(input_ids=tokens_tensors, 
                                            token_type_ids=segments_tensors,
                                            e1_mask=marked_e1,
                                            e2_mask=marked_e2,
                                            attention_mask=masks_tensors,
    ...
    if args.mode												 		== 'eval':
        if not check_file(tgt_checkpoint_file) :
