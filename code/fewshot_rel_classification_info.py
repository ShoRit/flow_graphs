...
def main(args):
    ...
    tgt_bertconfig = BertConfig.from_pretrained(
        args.bert_model, num_labels=len(set(tgt_train_label))
    )
    ...
    model = BertRGCNRelationClassifier.from_pretrained(
        args.bert_model, config=tgt_bertconfig, use_graph_data=use_graph_data
    )
    ...
    src_model_dict = {
        ...
        if k not in ["rel_classifier.weight", "rel_classifier.bias"]
    }
