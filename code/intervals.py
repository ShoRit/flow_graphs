...
from collections import MutableMapping, namedtuple, Counter
...
class IntervalMapping(MutableMapping):
    ...
    def iteritems(self, yield_nothing=False):
        ...
        keys = (Interval(a, b) for a, b in pairwise(chain([None], bounds, [None])))
        if yield_nothing:
    ...
    def __getitem__(self, key):
        ...
        if hasattr(key, "start") and hasattr(key, "stop"):
            ...
            rindex = bisect_left(bounds, stop) if stop is not None else len(bounds)
            if lindex != rindex:
                raise KeyError(key)
